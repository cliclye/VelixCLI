...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run velix",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for velix session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/velix\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found velix session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["velix", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting velix...")
        UI.println()

        const velixArgs = sessionId ? ["-s", sessionId] : []
        const velixProcess = Process.spawn(["velix", ...velixArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await velixProcess.exited
        if (code !== 0) throw new Error(`velix exited with code ${code}`)
      },
    })
  },
})
